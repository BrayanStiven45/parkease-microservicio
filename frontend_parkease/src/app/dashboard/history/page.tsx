...
export default function HistoryPage() {
    ...
    const fetchAdminHistory = useCallback(async () => {
        ...
        try {
            ...
            const fetchedRecords = (res?.records || []) as ParkingRecord[];
            fetchedRecords.sort((a, b) => new Date(b.exitTime ?? 0).getTime() - new Date(a.exitTime ?? 0).getTime());
    ...
    const fetchUserHistory = useCallback(() => {
        ...
        const load = async () => {
            try {
                const res = await ParkingAPI.getHistory();
                if (cancelled) return;
                const fetched = (res?.records || []) as ParkingRecord[];
                fetched.sort((a, b) => new Date(b.exitTime ?? 0).getTime() - new Date(a.exitTime ?? 0).getTime());
