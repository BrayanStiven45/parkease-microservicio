...
interface User {
  ...
  username: string;
  correo: string;
  role: string; // para saber si es admin
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchActive = async () => {
      try {
        const res = await ParkingAPI.getActive();
        const records: ParkingRecord[] = res?.records || [];
        setTotalParked(records.length);
        const times = records
          .map((r: any) => new Date(r.entryTime))
          .filter(
            (d: Date) => d instanceof Date && !isNaN(d.getTime())
          );
        setEntryTimes(times);
      } catch {
