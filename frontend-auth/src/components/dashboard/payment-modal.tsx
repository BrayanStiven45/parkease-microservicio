...
export default function PaymentModal({ 
  ...
  useEffect(() => {
    const fetchPoints = async () => {
      try {
        const loyaltyData = await parkingService.getLoyaltyPoints(record.plate);
  ...
  const handlePayment = async () => {
    ...
    try {
        const result = await parkingService.processPayment(userId, {
            recordId: record.id,
        ...
        toast({
            ...
            description: `Payment for ${record.plate} processed. Total: $${result.cost.toFixed(2)}. Points earned: ${costDetails.pointsEarned}.`,
        });
